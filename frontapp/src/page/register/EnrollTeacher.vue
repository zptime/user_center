<template>
  <div :class="{'noHeadContent':!regShowHead}">
    ...
    <div class="contentBox" v-cloak>
      <div class="info">
        <div class="warn">
          以下为教师注册信息，请确保填写的信息与向学校提供的信息保持一致
        </div>
        <div class="base-info">
        ...
        <div class="base-info">
          ...
          <div class="detail">
            <div class="detail-item detail-item-addr">
              <span class="detail-opt">
                <x-textarea v-model="baseInfo.address" placeholder="请填写准确的家庭地址，以免无法正确接受学校寄出去的通知文件" :rows="2" autosize></x-textarea>
              </span>
            </div>
            <div class="detail-item border-bottom-none">
              <span class="detail-title detail-title-long">孩子在本校就读</span>
              <span class="detail-opt sex-opt">
                <span @click="changeToggle(true)">
                  <img src="../../images/icon-select.png" v-if="baseInfo.is_school">
                  <img src="../../images/icon-deselect.png" v-else>
                </span>
                <span class="sex">是</span>
                <span @click="changeToggle(false)">
                  <img src="../../images/icon-deselect.png" v-if="baseInfo.is_school">
        ...
        <div class="base-info" v-for="(item,index) in childInfo" v-if="baseInfo.is_school">
          <div class="title">
            <span>孩子信息</span>
        ...
        <div class="btnCommon bottomButton boxShadowBlue" v-if="baseInfo.is_school" @click="addChild">+ 添加孩子</div>
      </div>
      <div class="btnOpt">
...
<script type="text/ecmascript-6">
  ...
  import { wxBindTeacher } from '../../service/getData'
  export default {
    data(){
      return {
        head:{
    ...
    methods:{
      ...
      dealInfo(){
        let showMsg = '';
        if(this.baseInfo.mobile==''){
          showMsg = '请输入联系方式';
        }else if(!(/^1[3|4|5|7|8][0-9]{9}$/.test(this.baseInfo.mobile))){
          showMsg = '请输入格式正确的手机号';
        }else if(this.baseInfo.is_school){
        ...
        if(showMsg){
          ...
          if(this.baseInfo.is_school){
            this.baseInfo.student_list_json = this.childInfo;
          }else{
            this.baseInfo.student_list_json = '';
          }
          this.TEACHERINFO(this.baseInfo);
          this.nextVerify();
        }
      },
      //“下一步”验证函数
      async nextVerify(){
        ...
        let res = await wxBindTeacher(tmp.mobile,tmp.address, JSON.stringify(tmp.student_list_json),
          tmp.messagecode,this.openid,this.sid,1);
        if(res.c==0){
          this.$router.push({
            name:'verify',
            query:{
